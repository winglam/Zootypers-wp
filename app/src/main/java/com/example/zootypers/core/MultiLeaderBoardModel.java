...
/**
 * The multiplayer leaderboard model class that keep track of every player's high score with name and score, and ranked
 * by to the highest score.
 *
 * @author nhlien93
 */
public class MultiLeaderBoardModel {
    ...
    /**
     * Return a List of score entries that is in the order of top to low scores Entry has field ( string and int ) that
     * stores name and scores The list will return a total of numOfEntries entries.
     *
    ...
    /**
     * Return a List of score entries that is in the order of high to low scores around your own score. The size of the
     * list will be defalut_entries with the goal being that the player's score is the center rank. Only time there is
     * less than default_entries is when there are less than default entries in the database
     *
     * @return a list of size numOfEntries with the top score entries
     */
    ...
    /**
     * @return highest possible relative rank with 1 being highest. If rank = 0 then user has no multiplayer score on
     * database
     */
