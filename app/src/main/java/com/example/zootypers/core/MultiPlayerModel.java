...
/**
 * The Model class for Multi-Player store a list of words for the UI to display. It keeps track of word and letter the
 * user has typed and updates the view accordingly.
 *
...
public class MultiPlayerModel extends PlayerModel {
    ...
    /**
     * Constructs a new SinglePlayerModel that takes in the ID of an animal and background, and also what the difficulty
    ...
    /**
     * Begins matchmaking the user by trying to find an opponent on the Parse database.
     *
    ...
    /**
     * Using the starting index of the match from online, make the words list array with LIST_SIZE words from the Parse
     * database
     *
     * @throws InternetConnectionException if disconnected from Internet
    ...
    private void checkIfInMatch() throws InternalErrorException {
        if (!match.getString(info.get("name")).equals(name)) {
            Log.e("Multiplayer", "got replaced by someone else, concurrency error");
            throw new InternalErrorException();
        }
    }
    ...
    /**
     * The typedLetter method handles what words and letter the user has typed so far and notify the view to highlight
     * typed letter or fetch a new word from the wordsList for the view to display accordingly.
     *
    ...
    /**
     * checks to see if the opponent is finished for a maximum of SCORE_TIMEOUT milliseconds. If the opponent is
     * finished then return true, false otherwise
     *
    ...
    /**
     * refreshes the match in a background thread and notifies the UI to update the opponent score after the refresh is
     * done.
     *
     * @modifies this
     */
