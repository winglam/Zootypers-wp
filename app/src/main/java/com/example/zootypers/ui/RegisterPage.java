...
/**
 * @author ZooTypers Handles new user registration
 */
public class RegisterPage extends Activity {
    ...
    public final void submitRegister(final View view) {
        ...
        if ((((usernameString.length() == 0) || (passwordString.length() == 0)) || (confirmPWString.length() == 0)) ||
    ...
    /**
     * Builds an alertdialog(popup) for a given title, message, and optional multiplayer option.
     *
     * @param title     String representing the title of the popup
     * @param message   String representing the message displayed in popup
     * @param goToTitle indicates whether popup should handle case where it goes back to titlepage
     */
    private void buildAlertDialog(final int title, final int message, final boolean goToTitle) {
        ...
        // set dialog message
        alertDialogBuilder.setMessage(message).setCancelable(false).setPositiveButton(R.string.close_alert,
     ...
     * @param errorFlag indicates whether text should be highlighted to indicate an error or not
     */
