...
public abstract class Player extends Activity implements Observer {
    ...
    /**
     * Called when the timer runs out; starts the post game screen activity with the correct data to pass.
     */
    public abstract void goToPostGame();

    /**
     * Called where there is a error. Quits the game and goes to the corresponding error page.
     */
    ...
    /**
     * Sets up the vibration so that if vibration in options is set to on then it vibrates whenever user types in the
     * incorrect letter.
     */
    ...
    /**
     * Set up the background music for the single & multi-player mode, so users can toggle on and off in the options
     * menu.
     *
        ...
        if (arg0 instanceof PlayerModel) {
            ...
            if (arg1 instanceof States.update) {
                ...
                if (change == States.update.FINISHED_WORD) {
                    ...
                    highlightWord(pM.getCurrWordIndex(), pM.getCurrWord(), pM.getCurrLetterIndex());
                    tv.setVisibility(TextView.INVISIBLE);
    ...
    /**
     * Highlights the letterIndex letter of the wordIndex word. letterIndex must not be beyond the scope of the word.
     *
